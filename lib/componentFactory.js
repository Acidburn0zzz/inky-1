...
let $ = require('cheerio');
let getAttrs = require('./util/getAttrs');

/**
 * Returns output for desired custom element
 * @param {object} element - Element as a Cheerio object.
 * @return {string} HTML converted from a custom element to table syntax.
 */
module.exports = function(element) {
  let inner = element.html();
  let attrs = getAttrs(element);
  ...
  case this.components.button:
    ...
    // Prepare optional universal attribute for the <a> element
    var universal = '';
    if (element.attr('universal')) {
      universal = ' universal="' + element.attr('universal') + '"';
    }

    // Prepare optional clicktracking attribute for the <a> element
    var clicktracking = '';
    if (element.attr('clicktracking')) {
      clicktracking = ' clicktracking="' + element.attr('clicktracking') + '"';
    }

    // If we have the href attribute we can create an anchor for the inner of the button;
    if (element.attr('href')) {
      inner = format('<a href="%s"%s%s%s>%s</a>', element.attr('href'), target, universal, clicktracking, inner);
    }
