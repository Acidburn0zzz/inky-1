...
module.exports = function(element) {
  ...
  switch (element[0].name) {
    ...
    case this.components.button:
      ...
      if (element.attr('href') && !element.attr('universal')) {
        inner = format('<a %s href="%s"%s>%s</a>', attrs, element.attr('href'), target, inner);
      }
      
      // Same, but including universal="true";
      if (element.attr('href') && element.attr('universal')) {
        inner = format('<a %s href="%s"%s universal="true">%s</a>', attrs, element.attr('href'), target, inner);
      }
